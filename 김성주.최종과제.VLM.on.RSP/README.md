...
├── captures/                  # 캡처된 프레임 저장 위치
└── llama.cpp/                 # llama.cpp 빌드 (추론 엔진)
```

## 현재 로컬 상태

- `llama.cpp`는 이미 빌드되어 있다.
- `llama.cpp/build/bin/llama-mtmd-cli`가 존재한다.
- 웹캠 장치가 `/dev/video0`에 있다.
- Python OpenCV가 설치되지 않은 환경에서는 스크립트가 `ffmpeg`로 대체 캡처한다.
- 전체 VLM GGUF 모델은 Hugging Face에서 자동 다운로드한다.
...
`llama-mtmd-cli`를 못 찾으면 빌드 경로를 확인한다.
